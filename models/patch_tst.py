...
class PatchTST(nn.Module):
  def __init__(self, n_token, input_dim, model_dim, num_heads, num_layers, output_dim):
    ...
    encoder_layers = nn.TransformerEncoderLayer(d_model=model_dim, nhead=num_heads, batch_first=True) # 128개로 늘려서 어텐션
  ...
  def forward(self, x):
    ...
    return F.sigmoid(output) # minmax Scaler전용 (scaling 안하면 x나 F.tanh)