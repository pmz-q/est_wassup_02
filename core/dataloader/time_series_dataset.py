...
class TimeseriesDataset(torch.utils.data.Dataset):
    def __init__(self, data:pd.DataFrame, window_size:int, prediction_size:int):
        self.data = data
    ...
    def __getitem__(self, idx:int):
        ...
        y = self.data[(idx+self.window_size):(idx+self.window_size+self.prediction_size)].T[0].T  # 예측하려는 미래 값(들)
        return x, y