...
# PatchTST: train & test together: ORIGIN MODEL
def main(cfg):
    ## load datasets
    ...
    # not using custom dataset
    # instead use torch dataset and will be using stride
    trn_ds = Dataset(trn_ds)
    val_ds = Dataset(val_ds)
    tst_ds = Dataset(tst_ds)
    ...
    model = Model(**{**model_params, "enc_in": n_features}).to(device)
...
if __name__ == "__main__":
    ...
    args = get_args_parser(config_type='train_patchtst_origin').parse_args()
    config = {}
